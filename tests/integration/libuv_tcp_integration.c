...
static int write_lua_fixture(const char *directory) {
  ...
  fputs("local btech_api = {\n"
        ...
        "offensive_battle_value_ref payload_ref real_max_speed section_status "
        ...
        "weapon_stat weight\",\n"
        "  character = \"list make_pilot_roll set_value threshold value\",\n"
        ...
        "}\n"
        "for package, names in pairs(btech_api) do\n"
        "  assert(type(btech[package]) == \"table\")\n"
        "  for name in names:gmatch(\"%S+\") do\n"
        "    assert(type(btech[package][name]) == \"function\")\n"
        "  end\n"
        "end\n"
  ...
  fputs("assert(type(mux.config.get(\"game_database\")) == \"string\")\n"
        ...
        "        assert(type(mux.world.create_room) == \"function\")\n"
        "        assert(type(mux.world.create_thing) == \"function\")\n"
  ...
  fputs(
      ...
      "      handler = function(ctx)\n"
      "        local room = mux.world.create_room({ name = \"Lua Room\" })\n"
      "        local styled_room = mux.world.create_room({\n"
      "          name = \"[fg=red]Lua Styled Room[/]\"\n"
      "        })\n"
      "        local thing = mux.world.create_thing({\n"
      "          name = \"Lua Thing\", location = room\n"
      "        })\n"
      "        local exit = mux.world.create_exit({\n"
      "          name = \"out;o\", location = room, destination = room\n"
      "        })\n"
      "        local link_destination_one = mux.world.create_room({\n"
      "          name = \"Lua Link Destination One\"\n"
      "        })\n"
      "        local link_destination_two = mux.world.create_room({\n"
      "          name = \"Lua Link Destination Two\"\n"
      "        })\n"
      "        local linked_exit = mux.world.create_exit({\n"
      "          name = \"Lua Persistently Linked Exit\", location = room\n"
      "        })\n"
      "        local unlinked_exit = mux.world.create_exit({\n"
      "          name = \"Lua Persistently Unlinked Exit\",\n"
      "          location = room, destination = link_destination_one\n"
      "        })\n"
      "        mux.world.link_exit(linked_exit, link_destination_one)\n"
      "        mux.world.link_exit(linked_exit, link_destination_two)\n"
      "        mux.world.link_exit(unlinked_exit, nil)\n"
      "        safe_thing = mux.world.create_thing({\n"
      "          name = \"Lua Safe\", location = ctx.enactor\n"
      "        })\n"
      "        assert(room.type == \"room\" and room.name == \"Lua Room\")\n"
      "        assert(styled_room.name == \"[fg=red]Lua Styled Room[/]\")\n"
      "        assert(thing.type == \"thing\" and thing.name == \"Lua "
      "Thing\")\n"
      "        assert(exit.type == \"exit\" and exit.name == \"out;o\")\n"
      "        assert(#room:contents() == 1 and room:contents()[1] == thing)\n"
      "        assert(#room:exits() == 3)\n"
      "        local missing_ok, missing_error = mux.error.pcall(\n"
      "          mux.world.link_exit, linked_exit)\n"
      "        assert(not missing_ok and missing_error.code == "
      "\"mux.arg.invalid\")\n"
      "        local source_ok, source_error = mux.error.pcall(\n"
      "          mux.world.link_exit, room, nil)\n"
      "        assert(not source_ok and source_error.code == "
      "\"mux.object.invalid\")\n"
      "        local target_ok, target_error = mux.error.pcall(\n"
      "          mux.world.link_exit, linked_exit, exit)\n"
      "        assert(not target_ok and target_error.code == "
      "\"mux.object.invalid\")\n"
      "        local going_destination = mux.world.create_thing({\n"
  ...
  fputs(
      ...
      "      pattern = \"^luateleport$\",\n"
      "      handler = function(ctx)\n"
      "        local source = mux.world.create_room({\n"
      "          name = \"Lua Teleport Source\"\n"
      "        })\n"
      "        local destination = mux.world.create_room({\n"
      ...
      "        local container = mux.world.create_thing({\n"
      "          name = \"Lua Teleport Container\", location = source\n"
      "        })\n"
      "        local child = mux.world.create_thing({\n"
      ...
      "        local going_object = mux.world.create_thing({\n"
  ...
  fputs("    {\n"
        ...
        "          name = \"Lua Zone Room\"\n"
        "        })\n"
        "        local room_zone = mux.world.create_room({\n"
        "          name = \"Lua Room Zone\"\n"
        "        })\n"
        "        local thing_zone = mux.world.create_thing({\n"
        "          name = \"Lua Persistent Zone\", location = room\n"
        "        })\n"
        "        local target = mux.world.create_thing({\n"
        "          name = \"Lua Persistently Zoned Thing\", location = room\n"
        "        })\n"
        "        mux.world.set_zone(target, thing_zone)\n"
        "        assert(mux.world.zone(target) == thing_zone)\n"
        "        assert(mux.world.zone(target.dbref) == thing_zone)\n"
        "        mux.world.set_zone(room, room_zone)\n"
        "        assert(mux.world.zone(room) == room_zone)\n"
        "        mux.world.set_zone(room, nil)\n"
        "        assert(mux.world.zone(room) == nil)\n"
        "        local missing_ok, missing_error = mux.error.pcall(\n"
        "          mux.world.set_zone, target)\n"
        "        assert(not missing_ok\n"
        "          and missing_error.code == \"mux.arg.invalid\")\n"
        "        local room_ok, room_error = mux.error.pcall(\n"
        "          mux.world.set_zone, target, room_zone)\n"
        "        assert(not room_ok\n"
        "          and room_error.code == \"mux.object.invalid\")\n"
        "        local exit = mux.world.create_exit({\n"
        ...
        "        })\n"
        "        local exit_ok, exit_error = mux.error.pcall(\n"
        "          mux.world.set_zone, target, exit)\n"
        "        assert(not exit_ok\n"
        "          and exit_error.code == \"mux.object.invalid\")\n"
        "        local going_zone = mux.world.create_thing({\n"
        "          name = \"Lua Going Zone\", location = room\n"
        "        })\n"
        "        mux.world.destroy(going_zone)\n"
        "        local zone_ok, zone_error = mux.error.pcall(\n"
        "          mux.world.set_zone, target, going_zone)\n"
        "        assert(not zone_ok\n"
  ...
  fputs(
      ...
      "        })\n"
      "        local cleared = mux.world.create_thing({\n"
      "          name = \"Lua Persistently Unparented Thing\", location = "
      "room\n"
      "        })\n"
      "        assert(mux.world.lua_parent(target) == \"default_thing.lua\")\n"
      "        local result = mux.world.set_lua_parent(target, "
      "\"example.lua\")\n"
      "        assert(result == nil)\n"
      "        assert(mux.world.lua_parent(target) == \"example.lua\")\n"
      "        assert(mux.world.lua_parent(target.dbref) == \"example.lua\")\n"
      "        mux.world.set_lua_parent(cleared, nil)\n"
      "        assert(mux.world.lua_parent(cleared) == nil)\n"
      "        local missing_ok, missing_error = mux.error.pcall(\n"
      "          mux.world.set_lua_parent, target)\n"
      "        assert(not missing_ok\n"
      "          and missing_error.code == \"mux.arg.invalid\")\n"
      "        local type_ok, type_error = mux.error.pcall(\n"
      "          mux.world.set_lua_parent, target, 7)\n"
      "        assert(not type_ok\n"
      "          and type_error.code == \"mux.arg.invalid\")\n"
      "        local nul_ok, nul_error = mux.error.pcall(\n"
      "          mux.world.set_lua_parent, target, \"example.lua\\0ignored\")\n"
      "        assert(not nul_ok\n"
  ...
  fputs(
      ...
      "      pattern = \"^luaflags$\",\n"
      "      handler = function(ctx)\n"
      "        local room = mux.world.create_room({ name = \"Lua Flags Room\" "
      "})\n"
      "        local target = mux.world.create_thing({\n"
