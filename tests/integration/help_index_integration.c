/* help_index_integration.c -- Indexes the isolated game/help fixture and
 * checks wizard_only filtering and the default article. */
...
int main(int argc, char *argv[]) {
  ...
  const char *game_directory = *(char *const *)checked_storage_at_const(
      argv, (size_t)argc, sizeof(*argv), 1);
  if (snprintf(help_directory, sizeof(help_directory), "%s/help",
               game_directory) >= (int)sizeof(help_directory))
    return 1;
