...
static void mech_enter_event(MuxEvent *e) {
  ...
  if (MechCarrying(mech) > 0)
    tmpm = btech_context_get_mech(mech_context(mech), MechCarrying(mech));
  obj_x = MechX(mech);
  obj_y = MechY(mech);
