...
mech_normal_to_hit_calculate(const MechNormalToHitRequest *request) {
  ...
  if ((float)rangecheck < range) {
    ...
    if (((float)weapon_ranges.minimum >= range) &&
      ...
      WeaponRangeToHitResult range_result = mech_c3_range_to_hit_calculate(
          &(C3RangeToHitRequest){.attacker = mech,
                                 .target = target,
                                 .section = section,
      ...
      WeaponRangeToHitResult range_result = mech_range_to_hit_calculate(
          &(WeaponRangeToHitRequest){.attacker = mech,
                                     .target = target,
                                     .section = section,
                                     .weapon_index = weapindx,
                                     .range = range,
