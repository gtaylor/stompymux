...
static void charge_mutual_resolve(Mech *mech, Mech *target) {
  int hit_group;
  int hitloc;
  bool isrear = false;
  ...
  if ((mech_class(target) == CLASS_MECH) && (mech_class(mech) != CLASS_MECH)) {
  ...
  if (has_bool_advantage(context, mech_pilot_dbref(target), "melee_specialist"))
    mech_damage++;
  /* Not able to do any damage */
  if (mech_damage <= 0) {
  ...
  targ_base_to_hit +=
      (has_bool_advantage(context, mech_pilot_dbref(target), "melee_specialist")
  ...
  if (target_charge) {
    if (targ_base_to_hit > 12) {
      mech_printf(target, MECHALL, "Charge: BTH %d\tYou choose not to charge.",
                  targ_base_to_hit);
      target_charge = 0;
    }
  }
  ...
  if (target_charge && targ_roll >= targ_base_to_hit) {
    ...
    if (btech_context_uses_new_charge_rules(context) &&
        btech_context_uses_technology_level_three_charge_rules(context)) {
      target_damage =
          charge_damage_calculate(&(ChargeDamageRequest){.moving = target,
                                                         .opponent = mech,
                                                         .mass_source = mech,
                                                         .uses_new_rules = true,
                                                         .divisor = 20});
...
void charge_mech(Mech *mech, Mech *target) {
  ...
  if (mech_charge_target_dbref(target) == mech_dbref(mech)) {
    charge_mutual_resolve(mech, target);
    return;
  }
