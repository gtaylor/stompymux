...
static void
mech_special_critical_effect_apply(const CriticalEffectRequest *request,
                                   int crit_hit) {
  ...
  const bool T_LOC_IS_ARM =
      ((HITLOC == LARM || HITLOC == RARM) && !mech_is_quad(wounded)) != 0;
  const bool T_LOC_IS_LEG =
...
bool mech_critical_effect_apply(const CriticalEffectRequest *request) {
  ...
  if (equipment_is_special(CRIT_TYPE)) {
    mech_special_critical_effect_apply(request, crit_hit);
  }
