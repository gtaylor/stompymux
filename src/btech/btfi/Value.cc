...
DV_VocabTable::TypedVocabTable *DV_VocabTable::builtin_table() {
...
DV_VocabTable::DV_VocabTable(bool read_only, FI_VocabIndex max_idx)
  ...
  // Add built-in values.
  static unique_ptr<Entry> entry_0(
      addStaticEntry(FI_VOCAB_INDEX_NULL, Value()));
}
...
size_t get_size_of(FI_ValueType value_type) {
  switch (value_type) {
...
void pow2_ceil_down(size_t &pow2_size, size_t exact_size) {
...
bool pow2_ceil_up(size_t &pow2_size, size_t exact_size) {
...
Value::~Value() { delete[] static_cast<char *>(value_buf); }
...
void Value::allocateValueBuffer(size_t new_buf_size) {
  if (new_buf_size == 0) {
    // Null assignment.
...
bool Value::setBufferType(FI_ValueType type, size_t count) {
...
void Value::setBufferValue(const void *src) {
  if (!value_buf) {
...
EncodingFormat choose_enc_format(FI_ValueType value_type) {
  switch (value_type) {
  case FI_VALUE_AS_UTF8:
...
FI_VocabIndex choose_enc_alg(FI_ValueType value_type) {
  switch (value_type) {
  case FI_VALUE_AS_SHORT:
    return FI_EA_SHORT;
...
void Value::setVocabTable(DV_VocabTable &new_vocab_table) {
  vocab_table = &new_vocab_table;
}

void Value::write(Encoder &encoder) const {
...
bool Value::read(Decoder &decoder) {
reparse:
...
void Value::encodeOctets(Encoder &encoder, FI_Length len) const {
...
bool Value::decodeOctets(Decoder &decoder, FI_Length len) {
  ...
  case ENCODE_WITH_ALGORITHM:
    ...
    if (!enc_alg->decode(&decoder.encoding_context, FI_Value::cast(*this), len,
                         r_buf)) {
      // FIXME: This Exception isn't really appropriate.
      throw UnsupportedOperationException();
    }
...
void Value::write_bit1_or_3(Encoder &encoder, bool is_bit1) const {
...
bool Value::read_bit1_or_3(Decoder &decoder, bool is_bit1) {
  FI_Octet bits;
...
void fi_destroy_value(FI_Value *obj) { delete obj; }

FI_ValueType fi_get_value_type(const FI_Value *obj) { return obj->getType(); }

size_t fi_get_value_count(const FI_Value *obj) { return obj->getCount(); }

const void *fi_get_value(const FI_Value *obj) { return obj->getBuffer(); }

int fi_set_value_type(FI_Value *obj, FI_ValueType type, size_t count) {
  return obj->setBufferType(type, count);
}

void fi_set_value(FI_Value *obj, const void *buf) { obj->setBufferValue(buf); }

void *fi_get_value_buffer(FI_Value *obj) { return obj->getBuffer(); }
