...
void die(const char *cause, const char *reason) {
  cerr << cause << ": " << reason << endl;
  throw 1;
}

template <typename T, typename U>
void test_max(T &table, const U &sample, FI_VocabIndex max = FI_ONE_MEG) {
...
void run_test() {
  ...
  if (idx1 != 16 || idx2 != 16 || idx3 != 17) {
    die("RA_VocabTable::getEntry(CharString)", "Incorrect indexes assigned");
  }

  if (*ra_vt1[idx1] != "hello world" || *ra_vt2[idx2] != "how now brown cow") {
    die("RA_VocabTable[FI_VocabIndex]", "Forward mapping failed");
  }
  ...
  //
  if (*ea_vt2[1] != &fi_ea_hexadecimal || *ea_vt2[2] != &fi_ea_base64 ||
      *ea_vt2[3] != &fi_ea_short || *ea_vt2[4] != &fi_ea_int ||
      *ea_vt2[5] != &fi_ea_long || *ea_vt2[6] != &fi_ea_boolean ||
      *ea_vt2[7] != &fi_ea_float || *ea_vt2[8] != &fi_ea_double ||
      *ea_vt2[9] != &fi_ea_uuid || *ea_vt2[10] != &fi_ea_cdata) {
    die("EA_VocabTable[FI_VocabIndex]", "Forward mapping failed");
  }
  ...
  if (idx1 != 1 || idx2 != 1 || idx3 != 1) {
    die("DS_VocabTable::getEntry(CharString)", "Incorrect indexes assigned");
  }

  if (*ds_vt1[FI_VOCAB_INDEX_NULL] != "" || *ds_vt1[idx1] != "how" ||
      *ds_vt2[idx2] != "now://brown" || *ds_vt3[idx3] != "cow") {
    die("DS_VocabTable[FI_VocabIndex]", "Forward mapping failed");
  }
  ...
  if (ds_vt1.getEntry("how").getIndex() != idx1 ||
    ...
    die("DS_VocabTable::getEntry(CharString)", "Reverse mapping failed");
  }
  ...
  if (*pfx_vt[FI_PFX_XML] != "xml") {
    die("PFX_DS_VocabTable[FI_VocabIndex]", "Forward mapping failed");
  }

  if (*nsn_vt[FI_NSN_XML] != "http://www.w3.org/XML/1998/namespace") {
    die("NSN_DS_VocabTable[FI_VocabIndex]", "Forward mapping failed");
  }

  if (pfx_vt.getEntry("xml").getIndex() != FI_PFX_XML) {
    die("PFX_DS_VocabTable.getEntry(CharString)", "Reverse mapping failed");
  }

  if (nsn_vt.getEntry("http://www.w3.org/XML/1998/namespace").getIndex() !=
      FI_NSN_XML) {
    die("NSN_DS_VocabTable.getEntry(CharString)", "Reverse mapping failed");
  }
  ...
  if (idx1 != 1 || idx2 != 1) {
    die("DN_VocabTable::getEntry(Name)", "Incorrect indexes assigned");
  }

  if (*dn_vt1[idx1]->pfx_part != *pfx_ref ||
      *dn_vt1[idx1]->nsn_part != *nsn_ref ||
      *dn_vt2[idx1]->local_part != *local_ref || dn_vt2[idx2]->pfx_part ||
      dn_vt2[idx2]->nsn_part || *dn_vt2[idx2]->local_part != *local_ref) {
    die("DN_VocabTable[FI_VocabIndex]", "Forward mapping failed");
  }
  ...
  if (dn_vt1.getEntry(n1).getIndex() != idx1 ||
    ...
    die("DN_VocabTable::getEntry(Name)", "Reverse mapping failed");
  }
  ...
  if (dn_vt1.getEntry(n3).getIndex() != FI_VOCAB_INDEX_NULL) {
    die("DN_VocabTable::getEntry(Name)", "Didn't clamp at maximum index");
  }
  ...
  if ((idx1 != local_ref.getIndex() + 1) || (idx2 != nsn_ref.getIndex() + 1) ||
...
void dv_table_test() {
  ...
  if (idx1 != 1 || idx2 != FI_VOCAB_INDEX_NULL || idx3 != 1) {
    die("DV_VocabTable::getEntry(Value)", "Incorrect indexes assigned");
  }
  ...
  if (dv_vt1[FI_VOCAB_INDEX_NULL]->getType() != FI_VALUE_AS_NULL ||
    ...
    die("DV_VocabTable[FI_VocabIndex]", "Forward mapping failed");
  }
  ...
  if (dv_vt1.getEntry(an_octet_value).getIndex() != idx1 ||
    ...
    die("DV_VocabTable::getEntry(Value)", "Reverse mapping failed");
  }
