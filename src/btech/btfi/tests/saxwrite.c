...
static void die_gen(const char *cause) {
  const FI_ErrorInfo *ei = fi_get_generator_error(gen);

  fprintf(stderr, "%s: %s\n", cause,
          ei->error_string ? ei->error_string : "???");
  exit(EXIT_FAILURE);
}
 ...
 */
static void start_element(const FI_Name *e_name, ...) {
...
static void end_element(const FI_Name *e_name) {
  /* End element.  */
...
static void characters(FI_ValueType type, size_t count, const void *buf) {
  /* Characters.  */
...
void write_test(FI_Vocabulary *const vocabulary, const char *const TEST_FILE) {
  ...
  start_element(en_how, an_cow, FI_VALUE_AS_UTF8, 3, "moo", 0);

  characters(FI_VALUE_AS_UTF8, 2, "\n\t");

  start_element(en_now, an_brown, FI_VALUE_AS_INT, 3, TERNARY, an_now,
                FI_VALUE_AS_FLOAT, 1, &PIE, 0);

  characters(FI_VALUE_AS_UTF8, 3, "\n\t\t");

  start_element(en_how, an_brown, FI_VALUE_AS_UTF8, 4, "oops", 0);
  ...
  start_element(en_now, an_now, FI_VALUE_AS_UTF8, 3, "cow", an_cow,
                FI_VALUE_AS_UTF8, 0, "", an_brown, FI_VALUE_AS_UTF8, 3, "moo",
                0);
  end_element(en_now);
