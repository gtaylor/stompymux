...
struct Comment : public Serializable {
  void write(Encoder &encoder) const { throw UnsupportedOperationException(); }
...
FI_tag_Generator::FI_tag_Generator(FI_Vocabulary &vocab)
    : fpout(0), buffer(0), vocabulary(vocab), element(document),
      characters(vocab.content_character_chunks) {
...
void fi_destroy_generator(FI_Generator *gen) { delete gen; }
...
FI_tag_Parser::FI_tag_Parser(FI_Vocabulary &vocab)
    : fpin(0), buffer(0), vocabulary(vocab), element(document),
      characters(vocabulary.content_character_chunks) {
  buffer = fi_create_stream(DEFAULT_BUFFER_SIZE);
  if (!buffer) {
    throw std::bad_alloc();
  }
...
void fi_destroy_parser(FI_Parser *parser) { delete parser; }
...
FI_Generator *GET_GEN(FI_ContentHandler *handler) {
  return static_cast<FI_Generator *>(handler->app_data_ptr);
}

// Helper routine to write out to buffers.
bool write_object(FI_Generator *gen, const Serializable &object) {
  // Serialize object.
  try {
  ...
  if (length > 0) {
    ...
    size_t w_len = fwrite(r_buf, sizeof(FI_Octet), length, gen->fpout);
    if (w_len != length) {
...
int gen_ch_startDocument(FI_ContentHandler *handler) {
...
int gen_ch_endDocument(FI_ContentHandler *handler) {
  FI_Generator *const gen = GET_GEN(handler);
...
int gen_ch_startElement(FI_ContentHandler *handler, const FI_Name *name,
...
int gen_ch_endElement(FI_ContentHandler *handler, const FI_Name *name) {
  FI_Generator *const gen = GET_GEN(handler);
...
int gen_ch_characters(FI_ContentHandler *handler, const FI_Value *value) {
  FI_Generator *const gen = GET_GEN(handler);
...
void read_file_octets(FI_Parser *parser) {
...
void parse_document(FI_Parser *parser) {
  // Pre-fill the buffer.
...
void parse_startDocument(FI_Parser *parser) {
...
void parse_endDocument(FI_Parser *parser) {
...
void parse_startElement(FI_Parser *parser) {
  ...
  if (!handler->startElement(handler, FI_Name::cast(name),
...
void parse_endElement(FI_Parser *parser) {
  parser->element.stop();
...
void parse_characters(FI_Parser *parser) {
  if (!parser->document.hasElements()) {
    // Character data is ony allowed inside of an element.
    throw IllegalStateException();
  }
...
// C.7
void Characters::write(Encoder &encoder) const {
  encoder.writeNext(CHARACTERS);

  w_value->write(encoder);
}

bool Characters::read(Decoder &decoder) { return r_value.read(decoder); }

// C.8
bool Comment::read(Decoder &decoder) {
