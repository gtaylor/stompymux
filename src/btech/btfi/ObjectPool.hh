...
template <typename T> struct ObjectPoolComp {
...
public:
  ...
  ~ObjectPool() {
    ...
    for (PoolMapIterator pp = pool_map.begin(); pp != pool_map.end(); ++pp) {
  ...
  public:
    ...
    operator const void *() const { return data_ptr; }

    const_reference operator*() const { return data_ptr->value; }

    const value_type *operator->() const { return &data_ptr->value; }

    // Get the metadata variable associated with the value.
    meta_value_type &metadata() const { return data_ptr->metadata; }
...
private:
  ...
  public:
    PoolData(ObjectPool &owner, const_reference value_ref)
        : value(value_ref), owner(owner), ref_count(0), is_interned(true) {
