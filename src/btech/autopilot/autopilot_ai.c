...
static void ai_path_collect_enemies(AiPathContext *path, Mech *mech,
  ...
  for (i = 0; i < battle_map_unit_count(map); i++) {
    ...
    if (mech_team(temp_mech) == mech_team(mech))
      continue;
    if (mech_range_to(mech, temp_mech) > 50.0F)
    ...
    location_simulation_initialize(&enemy->location, temp_mech);
    enemy->mech = temp_mech;
    enemy->out = false;
  }
}
static void ai_path_collect_friends(AiPathContext *path, Mech *mech,
  ...
  for (i = 0; i < battle_map_unit_count(map); i++) {
    ...
    location_simulation_initialize(&friend->location, temp_mech);
    friend->mech = temp_mech;
    friend->out = false;
  }
}
...
static AiPathScoreResult ai_path_score(const AiPathScoreRequest *request) {
  ...
  for (i = 0; i < OPTION_COUNT; i++) {
    ...
    candidate->location.dh =
        acceptable_degree(candidate->location.dh + option->heading_delta);
