...
typedef enum { EN_BLOB } ElementNameIndex;

static CachedName element_names[] = {{EN_BLOB, "blob", NULL},
                                     {-1, NULL, NULL}}; /* element_names[] */

#define GET_EN(x) (element_names[(x)].cached)

typedef enum { AN_ID, AN_CLASS } AttrNameIndex;

static CachedName attr_names[] = {{AN_ID, "id", NULL},
                                  {AN_CLASS, "class", NULL},
 ...
 * reallocation.
 */
typedef enum { VAR_ANY, VAR_INT_1 } ValueVariableIndex;
...
/* <blob> generation.  */
static int save_blob_walker(void *key, void *data, int depth, void *arg) {
  ...
  if (!fi_add_attribute(btdb_attrs, GET_AN(AN_ID), GET_VAR(VAR_INT_1))) {
  ...
  if (!fi_add_attribute(btdb_attrs, GET_AN(AN_CLASS), GET_VAR(VAR_INT_1))) {
    fputs("FIXME: BTDB: fi_add_attribute() error\n", stderr);
    return 0;
  }

  if (!btdb_gen_handler->startElement(btdb_gen_handler, GET_EN(EN_BLOB),
                                      btdb_attrs)) {
  ...
  /* Write blob content.  */
  if (!fi_set_value_type(GET_VAR(VAR_ANY), FI_VALUE_AS_OCTETS, blob_size)) {
  ...
  if (!btdb_gen_handler->characters(btdb_gen_handler, GET_VAR(VAR_ANY))) {
    fputs("FIXME: BTDB: characters() error\n", stderr);
    return 0;
  }

  /* Write </blob>.  */
  if (!btdb_gen_handler->endElement(btdb_gen_handler, GET_EN(EN_BLOB))) {
    fputs("FIXME: BTDB: startElement() error\n", stderr);
    return 0;
  }
...
int btdb_save_xcode(const FI_Name *name_xcode) {
  ...
  if (!btdb_gen_handler->startElement(btdb_gen_handler, name_xcode,
                                      btdb_attrs)) {
    fputs("FIXME: BTDB: startElement() error\n", stderr);
    return 0;
  }
...
int btdb_chars_in_xcode(const FI_Value *value) {
  if (in_blob_element && subelement_depth == 2) {
    ...
    diff_size =
        blob_content_size - (blob_content_cursor - (char *)blob_content);
