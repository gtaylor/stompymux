...
void mech_pickup(DbRef player, void *data, char *buffer) {
  ...
  DOCHECK(MechTeam(mech) != MechTeam(target) && Started(target),
...
void mech_attachcables(DbRef player, void *data, char *buffer) {
  ...
  DOCHECK(MechX(mech) != MechX(towMech) || MechY(mech) != MechY(towMech),
  ...
  DOCHECK((MechTeam(target) != MechTeam(mech)) && Started(target),
          "That unit can not be towed!");
