 ...
 */
static astar_node *auto_create_astar_node(short x, short y, short x_parent,
                                          short y_parent, short g_score,
                                          short h_score) {
...
int auto_astar_generate_path(AUTO *autopilot, MECH *mech, short end_x,
  ...
  while (!found_path) {
    ...
    parent_astar_node = (astar_node *)red_black_tree_search(open_list_by_score,
                                                            SEARCH_FIRST, NULL);
    ...
    for (i = 0; i < 360; i += 60) {
      ...
      if (CheckHexBit(open_list_bitfield, hexoffset)) {
        ...
        temp_astar_node = (astar_node *)red_black_tree_find(open_list_by_xy,
                                                            (void *)hexoffset);
  ...
  if (found_path) {
    ...
    if (!(temp_astar_node->x == MechX(mech) &&
      ...
      while (1) {
        ...
        hexoffset =
            HexOffSet(temp_astar_node->x_parent, temp_astar_node->y_parent);
  ...
  red_black_tree_release(open_list_by_score, (void *)astar_release, NULL);
  red_black_tree_destroy(open_list_by_xy);

  /* Destroy the closed list */
  red_black_tree_release(closed_list, (void *)astar_release, NULL);
