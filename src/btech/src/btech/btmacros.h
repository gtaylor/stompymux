...
#define AeroFreeFuel(a)                                                        \
  ((MechType(a) == CLASS_VTOL) && mudconf.btech_nofusionvtolfuel &&            \
...
#define SetCarriedCargo(a, b)                                                  \
  do {                                                                         \
    MechCarriedCargo(a) = (b);                                                 \
    SetCWCheck(a);                                                             \
  } while (0)
#define MechCarrying(a) (a)->rd.carrying
#define SetCarrying(a, b)                                                      \
  do {                                                                         \
    MechCarrying(a) = (b);                                                     \
    SetCWCheck(a);                                                             \
  } while (0)
...
#define HS_Size(a)                                                             \
  (MechType(mech) == CLASS_MECH                                                \
       ? (ClanMech(a) ? 2 : ((MechSpecials(a) & DOUBLE_HEAT_TECH) ? 3 : 1))    \
       : 1)
#define HS_Efficiency(a) (MechHasDHS(a) ? 2 : 1)
#define MechHasHeat(a)                                                         \
  (MechType(a) == CLASS_MECH || MechType(a) == CLASS_AERO ||                   \
   MechType(a) == CLASS_DS || MechType(a) == CLASS_SPHEROID_DS)
...
#define MechHasTurret(a)                                                       \
  ((MechType(a) == CLASS_VEH_GROUND || MechType(a) == CLASS_VEH_NAVAL ||       \
...
#define GetPartDamageFlags(a, b, c)                                            \
  MechSections(a)[b].criticals[c].weapDamageFlags
#define SetPartDamageFlags(a, b, c, d) GetPartDamageFlags(a, b, c) = d

#define GetPartDesiredAmmoLoc(a, b, c)                                         \
  MechSections(a)[b].criticals[c].desiredAmmoLoc
...
#define ClearTempNuke(mech, a, b)                                              \
  GetPartRBrand(mech, a, b) = GetPartBrand(mech, a, b)

#define PartIsNonfunctional(a, b, c)                                           \
  (PartIsDisabled(a, b, c) || PartIsBroken(a, b, c) || PartIsDestroyed(a, b, c))
#define PartIsDamaged(a, b, c) (GetPartFireMode(a, b, c) & DAMAGED_MODE)
#define DamagePart(a, b, c) (GetPartFireMode(a, b, c) |= DAMAGED_MODE)
#define UnDamagePart(a, b, c)                                                  \
  do {                                                                         \
    (GetPartFireMode(a, b, c) &= ~DAMAGED_MODE);                               \
    SetPartDamageFlags(a, b, c, 0);                                            \
    SetPartTempNuke(a, b, c, 0);                                               \
  } while (0)
...
#define PartIsBroken(a, b, c)                                                  \
  (GetPartFireMode(a, b, c) & (DESTROYED_MODE | BROKEN_MODE))
...
#define DestroyPart(a, b, c)                                                   \
  do {                                                                         \
    (GetPartFireMode(a, b, c) |= DESTROYED_MODE);                              \
    GetPartFireMode(a, b, c) &= ~(BROKEN_MODE | DISABLED_MODE | DAMAGED_MODE); \
    SetPartDamageFlags(a, b, c, 0);                                            \
    SetPartTempNuke(a, b, c, 0);                                               \
  } while (0)
#define UnDestroyPart(a, b, c)                                                 \
  do {                                                                         \
    (GetPartFireMode(a, b, c) &=                                               \
     ~(DESTROYED_MODE | HOTLOAD_MODE | DISABLED_MODE | BROKEN_MODE |           \
       DAMAGED_MODE));                                                         \
    SetPartDamageFlags(a, b, c, 0);                                            \
    SetPartTempNuke(a, b, c, 0);                                               \
  } while (0)
...
#define SetSectBreached(a, b)                                                  \
  do {                                                                         \
    MechSections(a)[b].config |= SECTION_BREACHED;                             \
    SetWCheck(a);                                                              \
  } while (0)
...
#define SetSectFlooded(a, b)                                                   \
  do {                                                                         \
    MechSections(a)[b].config |= SECTION_FLOODED;                              \
    SetWCheck(a);                                                              \
  } while (0)
#define UnSetSectFlooded(a, b)                                                 \
  do {                                                                         \
    MechSections(a)[b].config &= ~SECTION_FLOODED;                             \
    SetWCheck(a);                                                              \
  } while (0)
...
#define SetSectArmor(a, b, c)                                                  \
  do {                                                                         \
    (a)->ud.sections[b].armor = c;                                             \
    SetWCheck(a);                                                              \
  } while (0)
#define SetSectRArmor(a, b, c)                                                 \
  do {                                                                         \
    (a)->ud.sections[b].rear = c;                                              \
    SetWCheck(a);                                                              \
  } while (0)
#define SetSectInt(a, b, c)                                                    \
  do {                                                                         \
    (a)->ud.sections[b].internal = c;                                          \
    SetWCheck(a);                                                              \
  } while (0)
...
#define Immobile(a)                                                            \
  (!Started(a) || Uncon(a) || Fortified(a) || Blinded(a) ||                    \
   (MechMove(a) == MOVE_NONE) ||                                               \
   ((MechStatus(a) & FALLEN) &&                                                \
    ((MechType(a) != CLASS_MECH) && (MechType(a) != CLASS_MW))))
...
#define Sees360(mech)                                                          \
  ((MechMove(mech) == MOVE_NONE) || (MechType(mech) == CLASS_BSUIT))
...
#define MoveMod(mech)                                                          \
  (MechType(mech) == CLASS_MW                ? 3                               \
   : (MechIsBiped(mech) || MechIsQuad(mech)) ? 2                               \
                                             : 1)
...
#define AmmoMod(mech, loc, pos)                                                \
  ((!IsAmmo(GetPartType(mech, loc, pos)) ||                                    \
    ...
    GetPartAmmoMode(mech, loc, pos) & AC_PRECISION_MODE)                       \
       ? 1                                                                     \
       : (GetPartAmmoMode(mech, loc, pos) & AC_CASELESS_MODE ? .5 : 2))

#define JumpSpeed(mech, map)                                                   \
  ((InGravity(mech) && map)                                                    \
       ? (MechJumpSpeed(mech) * 100 / ((MAX(50, MapGravity(map)))))            \
       : MechJumpSpeed(mech))
#define JumpSpeedMP(mech, map) ((int)(JumpSpeed(mech, map) * MP_PER_KPH))

#define NotInWater(mech) (!(OnWater(mech)))
#define WaterBeast(mech)                                                       \
  (MechMove(mech) == MOVE_HULL || MechMove(mech) == MOVE_FOIL)

#define IsCrap(val)                                                            \
  (((val) == Special(ENDO_STEEL)) || ((val) == Special(FERRO_FIBROUS)) ||      \
   ((val) == Special(TRIPLE_STRENGTH_MYOMER)) ||                               \
   ((val) == Special(STEALTH_ARMOR)) ||                                        \
   ((val) == Special(HVY_FERRO_FIBROUS)) ||                                    \
   ((val) == Special(LT_FERRO_FIBROUS)))
...
#define OkayCritSect2(mech, sect, num, ok)                                     \
  (GetPartType(mech, sect, num) == (ok) &&                                     \
   !PartIsNonfunctional(mech, sect, num))
#define OkayCritSectS2(mech, sect, num, ok)                                    \
  OkayCritSect2(mech, sect, num, I2Special(ok))
#define MAPMOVEMOD(map)                                                        \
  ((map)->movemod > 0 ? (float)(map)->movemod / 100.0 : 1.0)
...
#define FaMechRange(mech, target)                                              \
  FindRange(MechFX(mech), MechFY(mech), MechFZ(mech), MechFX(target),          \
            MechFY(target), MechFZ(target))
...
#define ECMProtected(mech)                                                     \
  ((MechStatus2(mech) & ECM_PROTECTED) || ECMActive(mech) || PerECMActive(mech))
...
#define AngelECMProtected(mech)                                                \
  ((MechStatus2(mech) & ANGEL_ECM_PROTECTED) || AngelECMActive(mech))
