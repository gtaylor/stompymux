...
int CalculateLOSFlag(MECH *mech, MECH *target, MAP *map, int x, int y, int ff,
                     float hexRange) {
  ...
  if (hexRange > (((MechSpecials(mech) & AA_TECH) ||
                   (target && (MechSpecials(target) & AA_TECH)))
                      ? 180
                      : map->maxvis))
    return new_flag + MECHLOSFLAG_BLOCK;
...
#else
    if (MechToMech_LOSFlag(map, mech, target) &
