...
void FireWeapon(MECH *mech, MAP *mech_map, MECH *target, int LOS, int weapindx,
                int weapnum, int section, int critical, float enemyX,
  ...
  if (target) {
    if (InLineOfSight(target, mech, MechX(mech), MechY(mech), range))
      ...
      mech_printf(target, MECHALL,
                  ...
                  &MechWeapons[weapindx].name[3],
                  FindBearing(MechFX(target), MechFY(target), MechFX(mech),
                              MechFY(mech)));
  }
        ...
        if (target) {
          if ((MechType(target) == CLASS_BSUIT) &&
            ...
            if (roll >= baseToHit) {
              ...
              if (Elevation(mech_map, MechX(target), MechY(target)) <
                  (MechZ(target) - 2))
                tTryClear = 0;
            }
          }
