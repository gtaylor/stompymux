...
static int muxevent_mech_event[] = {0, 1, 0, 1, 1, 1, 1, 1, 1, 0, /* 0-9 */
  ...
  if (buffer && *buffer) {
    ...
    for (i = 0; i < MAX_EVENTS; i++)
      if (muxevent_exec_count[t[i]])
        notify_printf(
            player, "%-3d%-20s%10d %.3f%%", t[i], muxevent_names[t[i]],
            muxevent_exec_count[t[i]],
            ((float)100.0 * muxevent_exec_count[t[i]] / (tot_ev ? tot_ev : 1)));
