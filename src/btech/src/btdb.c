 ...
 */
typedef enum { EN_BTECHDB, EN_XCODE } ElementNameIndex;

static CachedName element_names[] = {{EN_BTECHDB, "btechdb", NULL},
                                     {EN_XCODE, "xcode", NULL},
                                     {-1, NULL, NULL}}; /* element_names[] */

#define GET_EN(x) (element_names[(x)].cached)

typedef enum { AN_VERSION } AttrNameIndex;

static CachedName attr_names[] = {{AN_VERSION, "version", NULL},
                                  {-1, NULL, NULL}}; /* attr_names[] */

#define GET_AN(x) (attr_names[(x)].cached)

typedef enum { VAR_INT_1 } ValueVariableIndex;

static CachedVariable value_vars[] = {
    {VAR_INT_1, FI_VALUE_AS_INT, 1, NULL},
    {-1, FI_VALUE_AS_NULL, 0, NULL}}; /* values[] */
...
static int real_save_btdb(FILE *fpout) {
  FI_Int32 version;
  ...
  if (!fi_add_attribute(btdb_attrs, GET_AN(AN_VERSION), GET_VAR(VAR_INT_1))) {
    fputs("FIXME: BTDB: fi_add_attribute() error\n", stderr);
    return 0;
  }

  if (!btdb_gen_handler->startElement(btdb_gen_handler, GET_EN(EN_BTECHDB),
                                      btdb_attrs)) {
  ...
  /* Write </btechdb>.  */
  if (!btdb_gen_handler->endElement(btdb_gen_handler, GET_EN(EN_BTECHDB))) {
...
static int real_load_btdb(FILE *fpin) {
  FI_Parser *parser;
...
static int check_btechdb_element(const FI_Attributes *attrs) {
  int ii;
...
static int btdb_startElement(FI_ContentHandler *handler, const FI_Name *name,
...
static int btdb_endElement(FI_ContentHandler *handler, const FI_Name *name) {
...
static int btdb_characters(FI_ContentHandler *handler, const FI_Value *value) {
  switch (element_context) {
  case EN_XCODE:
...
int init_btdb_element_names(CachedName *table) {
  ...
  for (ii = 0; table[ii].literal; ii++) {
    ...
    new_name = fi_create_element_name(vocabulary, table[ii].literal);
    if (!new_name) {
...
int init_btdb_attribute_names(CachedName *table) {
  ...
  for (ii = 0; table[ii].literal; ii++) {
    ...
    new_name = fi_create_attribute_name(vocabulary, table[ii].literal);
    if (!new_name) {
      fputs("FIXME: BTDB: Couldn't allocate name\n", stderr);
      fini_btdb_names(table);
      return 0;
...
int init_btdb_variables(CachedVariable *table) {
  ...
  for (ii = 0; table[ii].count; ii++) {
    ...
    if (!fi_set_value_type(new_var, table[ii].type, table[ii].count)) {
      fputs("FIXME: BTDB: Couldn't allocate variable\n", stderr);
