...
typedef struct InternalStructureRequest {
  Mech *mech;
  int location;
  int tonnage_index;
...
static int
expected_internal_structure(const InternalStructureRequest *request) {
  Mech *mech = request->mech;
  if (request->location == HEAD)
    return 3;
  int row = 0;
  while (internal_location_table(mech, row, 0) >= 0 &&
         request->location != internal_location_table(mech, row, 0))
    row++;
  if (internal_location_table(mech, row, 0) < 0)
    return 0;
  return internal_structure_value(
      (size_t)request->tonnage_index,
      (size_t)internal_location_table(mech, row, 1));
}
