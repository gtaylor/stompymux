...
/* mux_event_test.c - Standalone scheduler stress-test program. */
...
static void hellow(MuxEvent *e) {
  ...
  n = number(0, 10);
#endif
  mux_event_add_simple_arg(!n ? number(1, LONG_TIME) : number(1, SHORT_TIME),
...
int main(void) {
  ...
  for (i = 0; i < SIMULTANEOUS_EVENTS; i++)
    mux_event_add_simple_arg(i + 1, hellow,
