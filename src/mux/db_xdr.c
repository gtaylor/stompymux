...
static void mmdb_db_write(char *filename) {
  ...
  DO_WHOLE_DB(i) { mmdb_write_object(mmdb, i); }
    ...
    while (c) {
      ...
      if (c->numchannels > 0) {
        for (j = 0; j < c->numchannels; j++) {
          mmdb_write_opaque(mmdb, c->alias + j * 6,
                            strlen(c->alias + j * 6) + 1);
...
static int mmdb_db_read(char *filename) {
  ...
  dprintk("database XID: %08x%08x%08x%08x%08x", xid[0], xid[1], xid[2], xid[3],
          xid[4]);
  db_free();
