...
void do_say(DbRef player, DbRef cause, int key, char *message) {
  ...
  case SAY_POSE:
    ...
    break;
  case SAY_POSE_NOSPC:
    notify_all_from_inside(loc, player, tprintf("%s%s", Name(player), message));
    break;
  case SAY_EMIT:
...
static void page_return(DbRef player, DbRef target, const char *tag, int anum,
  ...
  str = attribute_parent_get(target, anum, &aowner, &aflags);
  if (*str) {
  ...
  for (p = (char *)strtok(oldlist, " "); p != NULL;
    ...
    if (ismessage)
      ...
      if (lookup_player(player, p, 1) != NOTHING) {
        safe_str(tprintf("%s, ", Name(lookup_player(player, p, 1))), namelist,
                 &bp);
      }
    }
  }
...
void do_page(DbRef player, DbRef cause, int key, char *tname, char *message) {
  ...
  if (!*message) {
    attribute_get_string(targetname, player, A_LASTPAGE, &aowner, &aflags);
    if (!*tname) {
      if (!*targetname)
  ...
  attribute_get_string(alias, player, A_ALIAS, &aowner, &aflags);
  if (*alias) {
    char *ap = aladd;
  ...
  if (((target = lookup_player(player, tname, 1)) == NOTHING) && n > 1) {
    ...
    if (target == NOTHING) {
      ...
      case ':':
        notify_with_cause(
            target, player,
            tprintf("From afar,%s %s %s", aladd, Name(player), message + 1));
        break;
      case ';':
        message++;
        notify_with_cause(
            target, player,
            tprintf("From afar,%s %s%s", aladd, Name(player), message));
        break;
      case '"':
        message++;
        [[fallthrough]];
  ...
  if (count == 1) {
    if (*buf1) {
      if (ispose != 1) {
        ...
        if (mp[0] == ':')
          notify_printf(player, "Long distance to %s: %s %s", buf1,
                        Name(player), mp + 1);
        else
          notify_printf(player, "Long distance to %s: %s%s", buf1, Name(player),
                        mp + 1);
      }
    }
  } else {
