...
struct confdata {
  ...
  int registeredonly; /* Only all logins with the REGISTERED flag. Middle ground
                         between @disable logins and Full Allow */
  int namechange_days;
  ...
  int btech_transported_unit_death; /* 1=Destroy units in a transport
                                     ...
                                     */
  int btech_mwpickup_action; /* 0 = 3030 style (Slave MW, @unlock, quiet @tel),
                                1 = TELE_LOUD (Triggers aenter), no slave, no
                                @unlock */
