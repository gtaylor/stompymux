...
static void muxevent_delete(MUXEVENT *e) {
  ...
  REMOVE_FROM_BIDIR_LIST(muxevent_first_in_type[(int)e->type], prev_in_type,
                         next_in_type, e);
...
void muxevent_run() { muxevent_tick += 1; }
...
int muxevent_last_type() { return last_muxevent_type; }

/* Initialize the events */

void muxevent_initialize() { dprintk("muxevent initializing"); }
...
int muxevent_count_data_data(int type, void *data, void *data2) {
  ...
  LoopEvent(e) if (e->data == data && e->data2 == data2) count++;
  return count;
}
...
int muxevent_last_type_data(int type, void *data) {
  ...
  if (type > last_muxevent_type)
    return last;
  LoopType(type, e) if (e->data == data) if ((t = (e->tick - muxevent_tick)) >
                                             last) last = t;
  return last;
}
...
int muxevent_first_type_data(int type, void *data) {
  ...
  if (type > last_muxevent_type)
    return last;
  LoopType(type,
           e) if (e->data == data) if ((t = (e->tick - muxevent_tick)) < last ||
                                       last < 0) if (t > 0) last = t;
  return last;
}
