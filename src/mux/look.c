...
static void look_atrs1(dbref player, dbref thing, dbref othing,
  ...
  for (ca = atr_head(thing, &as); ca; ca = atr_next(&as)) {
    ...
    if (check_exclude &&
        ((attr->flags & AF_PRIVATE) || nhashfind(ca, &mudstate.parent_htab)))
      continue;
  ...
  if (control || mudconf.read_rem_desc || nearby(player, thing)) {
    ...
    if (*temp) {
      if (Examinable(player, thing) || (aowner == Owner(player))) {
        view_atr(player, thing, atr_num(A_DESC), temp, aowner, aflags, 1);
      } else {
        show_desc(player, thing, 0);
      }
    }
...
void do_decomp(dbref player, dbref cause, int key, char *name, char *qual) {
        ...
        if (attr->flags & AF_IS_LOCK) {
          ...
          if (aflags & AF_LOCK) {
            notify_printf(player, "@lock %s/%s",
                          strip_ansi_r(new, thingname, strlen(thingname)),
                          buff);
          }

          for (np = indiv_attraccess_nametab; np->name; np++) {

            if ((aflags & np->flag) && check_access(player, np->perm) &&
  ...
  if (qual && *qual) {
    ...
    case TYPE_EXIT:
      StringCopy(thingname, Name(thing));
      notify_printf(player, "@open %s", translate_string(Name(thing), 1));
  ...
  for (ca = atr_head(thing, &as); ca; ca = atr_next(&as)) {
    ...
    if (Read_attr(player, thing, attr, aowner, aflags)) {
      if (attr->flags & AF_IS_LOCK) {
        ...
        ltext = unparse_boolexp_decompile(player, boolexp);
        free_boolexp(boolexp);
        notify_printf(player, "@lock/%s %s=%s", attr->name, thingname, ltext);
      } else {
        StringCopy(buff, attr->name);
        notify_printf(player, "%c%s %s=%s", ((ca < A_USER_START) ? '@' : '&'),
                      buff, strip_ansi_r(new, thingname, strlen(thingname)),
                      got);
  ...
  if (Parent(thing) != NOTHING)
    notify_printf(player, "@parent %s=#%d",
                  strip_ansi_r(new, thingname, strlen(thingname)),
                  Parent(thing));
  ...
  if (Zone(thing) != NOTHING)
    notify_printf(player, "@chzone %s=#%d",
                  strip_ansi_r(new, thingname, strlen(thingname)), Zone(thing));
