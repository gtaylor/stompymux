...
int helpindex_read(HashTable *htab, char *filename) {
  help_indx entry;
  char *p;
  int count;
  FILE *fp;
  ...
  while ((fread((char *)&entry, sizeof(help_indx), 1, fp)) == 1) {
     ...
     *
     * * Substrings already added will be rejected by hash_table_add.
     */
    ...
    while (p > entry.topic) {
      p--;
      if (!isspace(*p)) {
        if ((hash_table_add(entry.topic, (int *)htab_entry, htab)) == 0)
          count++;
        else {
          free(htab_entry->key);
          free(htab_entry);
...
void helpindex_init(void) {
  hash_table_initialize(&mudstate.help_htab, 400 * HASH_FACTOR);
  hash_table_initialize(&mudstate.wizhelp_htab, 400 * HASH_FACTOR);

  helpindex_load(NOTHING);
}

void help_write(DbRef player, char *topic, HashTable *htab, char *filename,
                int eval) {
  FILE *fp;
