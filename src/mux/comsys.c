...
void init_chantab(void) { hashinit(&mudstate.channel_htab, 30 * HASH_FACTOR); }
...
void load_comsystem(FILE *fp) {
  ...
  for (i = 0; i < nc; i++) {
    ...
    if (new) { /* V1 or higher */
      ...
      dummy = 0;
      k = fscanf(fp,
                 new == 1 ? "%d %d %d %d %d %d %d %d\n"
                          : "%d %d %d %d %d %d %d %d %d\n",
                 &(ch->type), &(ch->temp1), &(ch->temp2), &(ch->charge),
                 &(ch->charge_who), &(ch->amount_col), &(ch->num_messages),
                 &(ch->chan_obj), &dummy);
      ...
      if (fscanf(fp, "%d %d %d %d %d %d %d %d %d %d\n", &(ch->type), &(dummy),
                 &(ch->temp1), &(ch->temp2), &(dummy), &(ch->charge),
                 &(ch->charge_who), &(ch->amount_col), &(ch->num_messages),
                 &(ch->chan_obj)) != 10)
        return;
    }
    ...
    if (ch->num_users > 0) {
      ch->users =
          (struct comuser **)calloc(ch->max_users, sizeof(struct comuser *));
...
void save_comsystem(FILE *fp) {
  ...
  for (ch = (struct channel *)hash_firstentry(&mudstate.channel_htab); ch;
    ...
    fprintf(fp, "%d %d %d %d %d %d %d %d %d\n", ch->type, ch->temp1, ch->temp2,
            ch->charge, ch->charge_who, ch->amount_col, ch->num_messages,
            ch->chan_obj, k);
