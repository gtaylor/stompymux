...
struct mmdb_t *mmdb_open_read(char *filename) {
  ...
  mmdb->base = mmap(NULL, mmdb->length, PROT_READ, MAP_SHARED, mmdb->fd, 0);
  if (!mmdb->base) {
    close(mmdb->fd);
    free(mmdb);
    return NULL;
...
struct mmdb_t *mmdb_open_write(char *filename) {
  ...
  fsync(mmdb->fd);
  mmdb->base =
      mmap(NULL, mmdb->length, PROT_READ | PROT_WRITE, MAP_SHARED, mmdb->fd, 0);
...
void mmdb_write_uint(struct mmdb_t *mmdb, unsigned int val) {
  printk(
      "This function is deprecated, please use mmdb_write_uint32 instead.\n");
