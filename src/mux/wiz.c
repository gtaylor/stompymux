...
void do_toad(dbref player, dbref cause, int key, char *toad, char *newowner) {
  ...
  snprintf(buf, MBUF_SIZE, "%s has been turned into a slimy toad!",
           Name(victim));
...
static void show_quota(dbref player, dbref victim) {
  ...
  if (!Free_Quota(victim))
    notify_quiet(player,
                 tprintf("%-16s Quota: %9d  Used: %9d", Name(victim), aq, rq));
  else
    notify_quiet(
        player, tprintf("%-16s Quota: UNLIMITED  Used: %9d", Name(victim), rq));
}
