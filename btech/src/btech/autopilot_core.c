...
void auto_load_commands(FILE *file, AUTO *autopilot) {
  ...
  if (fread(&tmpb, 1, 1, file) != 1) {
    fprintf(stderr,
            "Unable to read START for reading data"
            " for AI #%ld\n",
            autopilot->mynum);
    fflush(stderr);
    exit(1);
  ...
  if (tmpb != AI_COMMAND_DLLIST_START) {
    fprintf(stderr,
            "Unable to locate START for reading data"
            " for AI #%ld\n",
            autopilot->mynum);
    fflush(stderr);
    exit(1);
  ...
  if (fread(&tmpb, 1, 1, file) != 1) {
    fprintf(stderr,
            "Unable to read END for reading data"
            " for AI #%ld\n",
            autopilot->mynum);
    fflush(stderr);
    exit(1);
  ...
  if (tmpb != AI_COMMAND_DLLIST_END) {
    fprintf(stderr,
            "Unable to locate END for reading data"
            " for AI #%ld\n",
            autopilot->mynum);
    fflush(stderr);
    exit(1);
