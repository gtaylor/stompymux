...
#define BTHBASE(m, t, n)                                                       \
  do {                                                                         \
    if (t)                                                                     \
      snprintf(buf, LBUF_SIZE, "#%ld -> #%ld: Base %d", m->mynum, t->mynum,    \
               n);                                                             \
    else                                                                       \
      snprintf(buf, LBUF_SIZE, "#%ld -> (hex): Base %d", m->mynum, n);         \
    baseToHit = n;                                                             \
...
int FindNormalBTH(MECH *mech, MAP *mech_map, int section, int critical,
  ...
  *c3Ref = -1;
  ...
  if (rangecheck < range) {
    ...
    if ((MechWeapons[weapindx].min >= range) &&
    ...
    } else if (HasC3(mech) && !C3Destroyed(mech) && !AnyECMDisturbed(mech) &&
      ...
      wRangeBracket = FindBTHByRange(mech, target, section, weapindx, range,
                                     wFireMode, wAmmoMode, &rbth);
      BTHADD("Range", rbth);
    }
  }
  ...
  if (target && MechInfantrySpecials(target) & STEALTH_TECH) {
    ...
    } else if (MechInfantrySpecials(target) & FC_INFILTRATORII_STEALTH_TECH) {
      if (wRangeBracket == RANGE_SHORT)
  ...
  if (target && (MechType(target) == CLASS_VTOL) &&
      (fabs(MechSpeed(target)) > 0.0 || fabs(MechVerticalSpeed(target)) > 0.0))
    BTHADD("TargetVTOL", 1);
  ...
  if (target) {
    ...
    if (target && !IsMissile(weapindx) && MechAim(mech) == HEAD &&
      ...
      if ((GetPartFireMode(mech, section, critical) & ON_TC) &&
          !(MechCritStatus(mech) & TC_DESTROYED) && !(wAmmoMode & LBX_MODE)) {
...
int FindArtilleryBTH(MECH *mech, int section, int weapindx, int indirect,
  ...
  if (indirect) {
    ...
    if (indirect &&
        (MechSpotter(mech) == -1 || MechSpotter(mech) == mech->mynum))
      baseToHit += 1;
  } else
    baseToHit -= 2;
