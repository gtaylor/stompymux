...
FI_VocabIndex DN_VocabTable::DynamicNameEntry::acquireIndex() {
  ...
  if (value.pfx_part && (!value.pfx_part.hasIndex() ||
                         value.pfx_part.getIndex() == FI_VOCAB_INDEX_NULL)) {
    return FI_VOCAB_INDEX_NULL;
  }

  if (value.nsn_part && (!value.nsn_part.hasIndex() ||
                         value.nsn_part.getIndex() == FI_VOCAB_INDEX_NULL)) {
    return FI_VOCAB_INDEX_NULL;
  }

  if (!value.local_part || value.local_part.getIndex() == FI_VOCAB_INDEX_NULL) {
    return FI_VOCAB_INDEX_NULL;
  }
...
void fi_destroy_name(FI_Name *name) { delete name; }
