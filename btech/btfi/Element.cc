...
void Element::start(const NSN_DS_VocabTable::TypedEntryRef &ns_name) {
...
void Element::write(Encoder &encoder) const {
...
bool Element::read(Decoder &decoder) {
  switch (serialize_mode) {
  case SERIALIZE_START:
    switch (r_state) {
...
void Element::write_start(Encoder &encoder) const {
...
void Element::write_namespace_attributes(Encoder &encoder) const {
...
void Element::write_attributes(Encoder &encoder) const {
  ...
  for (int ii = 0; ii < w_attrs->getLength(); ii++) {
    ...
    encoder.writeAttribute(w_attrs->getName(ii), w_attrs->getValue(ii));
  }
...
bool Element::read_start(Decoder &decoder) {
  FI_Octet bits;
...
bool Element::read_namespace_attributes(Decoder &decoder) {
  if (doc.hasElements()) {
  ...
  for (;;) {
    ...
    case RESET_ATTR_STATE:
      ...
      case FI_BITS(1, 1, 1, 1, 0, 0, /*0*/, /*0*/):
        ...
        if (decoder.getBits() != FI_BITS(1, 1, 1, 1, 0, 0, 0, 0)) {
          // Not a valid Fast Infoset.
          throw IllegalStateException();
        }
    ...
    case END_ATTR_STATE:
      ...
      if ((decoder.getBits() & FI_BIT_1) || (decoder.getBits() & FI_BIT_2)) {
        // Not a valid Fast Infoset.
        throw IllegalStateException();
      }
      return true;
...
bool Element::read_attributes(Decoder &decoder) {
