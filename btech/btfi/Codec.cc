...
void Codec::setEncodingAlgorithm(FI_VocabIndex idx) {
 ...
 * Byte-level I/O.
 */
FI_Octet *Encoder::getWriteBuffer(FI_Length length) {
...
const FI_Octet *Decoder::getReadBuffer(FI_Length length) {
...
bool Decoder::skipLength(FI_Length &length) {
...
void Encoder::writeBits(unsigned int num_bits, FI_Octet octet_mask) {
...
bool Decoder::readBits(unsigned int num_bits) {
  assert(num_bits <= 8 - num_partial_bits);
...
void Encoder::writeNext(ChildType child_type) {
...
bool Decoder::readNext(ChildType &child_type) {
  // Children always start on the first bit of an octet, but the most
  // recent bit may either be the fourth or eight bit (C.2.11.1,
  // C.3.7.1), possibly requiring padding.
  switch (getBitOffset()) {
...
const char *const xml_decl[] = {
    ...
    "<?xml version='1.1' encoding='finf' standalone='yes'?>"}; // xml_decl[]

} // anonymous namespace

void Encoder::writeXMLDecl(XMLVersion version, FI_Ternary standalone) {
  int ii;
...
bool Decoder::readXMLDecl() {
  ...
  case 1:
    ...
    if (r_buf[1] != '?' || r_buf[2] != 'x' || r_buf[3] != 'm' ||
        r_buf[4] != 'l') {
      // Not a valid Fast Infoset.
      throw IllegalStateException();
    }
...
void Encoder::writeAttribute(const DN_VocabTable::TypedEntryRef &name,
                             const Value &value) {
...
bool Decoder::readAttribute(DN_VocabTable::TypedEntryRef &name, Value &value) {
  switch (super_step) {
  case 0:
...
bool Decoder::readNSAttribute(NSN_DS_VocabTable::TypedEntryRef &ns_name) {
...
// C.13
void Encoder::writeIdentifier(const DS_VocabTable::TypedEntryRef &id) {
...
bool Decoder::readIdentifier(DS_VocabTable &string_table,
...
reparse:
  ...
  case 3:
    ...
    // Enter literal into vocabulary table (7.13.7).
    id = string_table.getEntry(
        CharString(reinterpret_cast<const char *>(r_buf), saved_len));
    id.getNewIndex();
    break;
...
// C.17
void Encoder::writeName_bit2(const DN_VocabTable::TypedEntryRef &name) {
...
bool Decoder::readName_bit2(DN_VocabTable &name_table,
...
reparse:
  ...
  case 2:
    ...
    if (!readIdentifier(vocabulary->prefixes, saved_pfx_part)) {
      return false;
    }
  ...
  case 3:
    ...
    if (!readIdentifier(vocabulary->namespace_names, saved_nsn_part)) {
      return false;
    }
  ...
  case 4:
    ...
    if (!readIdentifier(vocabulary->local_names, saved_local_part)) {
      return false;
    }

    // Enter literal into vocabulary table (7.13.7).
    name = name_table.getEntry(
        Name(saved_local_part, saved_nsn_part, saved_pfx_part));
    name.getNewIndex();
    break;
  }
...
// C.18
void Encoder::writeName_bit3(const DN_VocabTable::TypedEntryRef &name) {
...
bool Decoder::readName_bit3(DN_VocabTable &name_table,
                            DN_VocabTable::TypedEntryRef &name) {
  FI_Octet bits;
...
void Encoder::writeNonEmptyOctets_len_bit2(FI_PInt32 len) {
...
bool Decoder::readNonEmptyOctets_len_bit2(FI_PInt32 &len) {
  assert(getBitOffset() == 1); // C.22.2
...
// C.23.3
// Note that this only writes the length of the octet string.
void Encoder::writeNonEmptyOctets_len_bit5(FI_PInt32 len) {
...
bool Decoder::readNonEmptyOctets_len_bit5(FI_PInt32 &len) {
  assert(getBitOffset() == 4); // C.23.2
...
// C.24.3
// Note that this only writes the length of the octet string.
void Encoder::writeNonEmptyOctets_len_bit7(FI_PInt32 len) {
...
bool Decoder::readNonEmptyOctets_len_bit7(FI_PInt32 &len) {
  assert(getBitOffset() == 6); // C.24.2
...
// C.25
void Encoder::writePInt20_bit2(FI_PInt20 val) {
...
bool Decoder::readPInt20_bit2(FI_PInt20 &val) {
  assert(getBitOffset() == 1); // C.25.1
...
// C.26
void Encoder::writeUInt21_bit2(FI_UInt21 val) {
...
bool Decoder::readUInt21_bit2(FI_UInt21 &val) {
  assert(getBitOffset() == 1); // C.26.1
...
// C.27
void Encoder::writePInt20_bit3(FI_PInt20 val) {
...
bool Decoder::readPInt20_bit3(FI_PInt20 &val) {
  assert(getBitOffset() == 2); // C.27.1
...
// C.28
void Encoder::writePInt20_bit4(FI_PInt20 val) {
...
bool Decoder::readPInt20_bit4(FI_PInt20 &val) {
  assert(getBitOffset() == 3); // C.28.1
...
// C.29
void Encoder::writePInt8(FI_PInt8 val) {
...
bool Decoder::readPInt8(FI_PInt8 &val) {
  FI_PInt8 tmp_val;
