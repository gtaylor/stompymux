...
public:
  ...
  // root-level table).
  VocabTable *getParent() const { return parent; }
  ...
  public:
    ...
    bool hasIndex() const { return entry->hasIndex(); }

    // Invoke getIndex() on the associated Entry.
    FI_VocabIndex getIndex() const { return entry->getIndex(); }
    ...
    operator const void *() const { return entry; }
...
protected:
  ...
  VocabTable(bool read_only, FI_VocabIndex max_idx)
      : parent(0), read_only(read_only), max_idx(max_idx), base_idx(1),
        last_idx(0) {}
  ...
  protected:
    ...
    virtual FI_VocabIndex acquireIndex() { return owner.acquireIndex(this); }
 ...
 */
template <typename T> class TypedVocabTable : public VocabTable {
public:
  class TypedEntryRef;
...
public:
  ...
  public:
    ...
    TypedEntryRef(const TypedEntryRef &src) : EntryRef(src.entry) {}
    ...
    const_reference operator*() const { return getTypedEntry()->value; }

    const value_type *operator->() const { return &getTypedEntry()->value; }
    ...
    bool operator<(const TypedEntryRef &src) const {
      ...
      if (!entry) {
        ...
        // L < R ?
        return getTypedEntry()->value < src.getTypedEntry()->value;
      }
    }
...
protected:
  TypedVocabTable(bool read_only, FI_VocabIndex max_idx)
      : VocabTable(read_only, max_idx), typed_parent(0), own_entry_pool(true),
  ...
  TypedEntry *addStaticEntry(FI_VocabIndex idx, const_reference value) {
    ...
    TypedEntry *const entry = new StaticTypedEntry(*this, pool_ptr, idx);
    pool_ptr.metadata() = entry;

    if ((base_idx + vocabulary.size()) != idx || acquireIndex(entry) != idx) {
      throw AssertionFailureException();
    }
  ...
  protected:
    TypedEntry(TypedVocabTable &owner, const EntryPoolPtr &pool_ptr)
  ...
  public:
    StaticTypedEntry(TypedVocabTable &owner, const EntryPoolPtr &pool_ptr,
                     FI_VocabIndex idx)
        : TypedEntry(owner, pool_ptr), static_idx(idx) {}

    bool hasIndex() const { return true; }

    FI_VocabIndex getIndex() { return static_idx; }
 ...
 */
template <typename T> class DynamicTypedVocabTable : public TypedVocabTable<T> {
protected:
...
protected:
  DynamicTypedVocabTable(bool read_only, FI_VocabIndex max_idx)
      : TypedVocabTable(read_only, max_idx), dynamic_root(0) {}

  DynamicTypedVocabTable(bool read_only, TypedVocabTable *parent)
      : TypedVocabTable(read_only, parent), dynamic_root(0) {}
  ...
  virtual DynamicTypedEntry *createEntryObject(const EntryPoolPtr &pool_ptr) {
  ...
  public:
    ...
    void resetIndex() { has_cached_idx = false; }
