...
static void die_parser(const char *cause) {
...
static int startDocument(FI_ContentHandler *handler) {
  puts("(START DOCUMENT)");
  return 1;
}

static int endDocument(FI_ContentHandler *handler) {
  puts("\n(END DOCUMENT)");
  return 1;
}
...
static void print_name(const FI_Name *name) {
...
static void print_value_content(const FI_Value *value) {
...
static void print_attr_value(const FI_Value *value) {
...
static int startElement(FI_ContentHandler *handler, const FI_Name *name,
...
static int endElement(FI_ContentHandler *handler, const FI_Name *name) {
  if (is_open) {
    fputs(" />", stdout);
    is_open = 0;
  } else {
...
static int characters(FI_ContentHandler *handler, const FI_Value *value) {
  if (is_open) {
    /* Close the most recent element.  */
    fputs(">", stdout);
    is_open = 0;
...
void read_test(FI_Vocabulary *const vocabulary, const char *const TEST_FILE) {
  FI_Name *e_name;
